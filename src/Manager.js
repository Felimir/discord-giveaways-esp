...
class GiveawaysManager extends EventEmitter {
    ...
    constructor(client, options) {
        super();
        if (!client) throw new Error('Client es una opción requerida.');
        /**
         * The Discord Client
         * @type {Client}
         */
     ...
     */
        ...
        if (!giveawayData) {
            return reject('No se encontró un sorteo con la ID ' + messageID + '.');
        }
    ...
    start(channel, options = {}) {
        return new Promise(async (resolve, reject) => {
            if (!this.ready) {
                return reject('El Administrador no está listo aún.');
            }
            if (!options.messages) {
                options.messages = defaultGiveawaysMessages;
            }
            if (!channel || !channel.id) {
                return reject(`El canal no es de la clase GuildChannel. (valor=${channel})`);
            }
            if (!options.time || isNaN(options.time)) {
                return reject(`options.time no es un número. (valor=${options.time})`);
            }
            if (!options.prize) {
                return reject(`options.prize no es un string. (valor=${options.prize})`);
            }
            if (!options.winnerCount || isNaN(options.winnerCount)) {
                return reject(`options.winnerCount no es un número. (valor=${options.winnerCount})`);
            }
    ...
    reroll(messageID, options = {}) {
        ...
        let giveawayData = this.giveaways.find(g => g.messageID === messageID);
        if (!giveawayData) {
            return reject('No se encontró el sorteo con la ID ' + messageID + '.');
        }
        let giveaway = new Giveaway(this, giveawayData);
    ...
    delete(messageID, doNotDeleteMessage) {
        return new Promise(async (resolve, reject) => {
            ...
            let giveaway = new Giveaway(this, giveawayData);
            if (!giveaway.channel) {
                return reject('No se pudo encontrar el canal del sorteo con la ID ' + giveaway.messageID + '.');
            }
        ...
        if (!storageExists) {
            ...
            try {
                ...
                if (Array.isArray(giveaways)) {
                    ...
                    throw new SyntaxError('El archivo de almacenamiento no está correctamente formado.');
                }
            } catch (e) {
                if (e.message === 'Unexpected end of JSON input') {
                    throw new SyntaxError('El archivo de almacenamiento no está correctamente formado.', e);
                } else {
                    throw e;
                }
            }
